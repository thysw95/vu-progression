                ...
                if #weaponUnlocks.unlocks > 0 then
                    for _, unlock in pairs(weaponUnlocks.unlocks) do
                        if CONFIG.UnlockNotifications.enabled == true and weapKills == unlock.killsRequired then
							...
							NetEvents:SendTo('showmessage',player,{'',message,CONFIG.UnlockNotifications.duration})
                           --NetEvents:SendTo('PlayUnlockSound', player, CONFIG.UnlockNotifications.soundPaths.weapAttachUnlock)
							NetEvents:SendTo('playsound',player,{'UI_Awards_Unlock',1,1})
                            break
                        end
...
function PlayerLevelUp(player, levelType, level, unlockName)
    if player ~= nil then
        ...
        if CONFIG.UnlockNotifications.enabled == true then
            ...
            --ChatManager:Yell(message, CONFIG.UnlockNotifications.duration, player)
			local id = string.format("%03d", math.min(math.max(level, 1), 100))
			NetEvents:SendTo('showmessage',player,{'fb://UI/Art/Persistence/Ranks/Icons/RankS'..id,message,CONFIG.UnlockNotifications.duration})
            --NetEvents:SendTo('PlayUnlockSound', player, CONFIG.UnlockNotifications.soundPaths.levelUp)
			NetEvents:SendTo('playsound',player,{'UI_Award_RankUp',1,1})
        end
    end
end
...
function ChatCommand(player, recipientMask, message)
    ...
    if string.lower(message) == "!level" or string.lower(message) == "!score" then
		...
		NetEvents:SendTo('showmessage',player,{'',"[= CLASS LEVELS =]",4})
        for i, className in pairs(classNames) do
			...
			NetEvents:SendTo('showmessage',player,{'',msg,4})
        end
    ...
    elseif string.lower(message):sub(1, 3) == "!vs" then
       ...
       --[[ ChatManager:SendMessage(
		...
		NetEvents:SendTo('showmessage',player,{'',"Your vehicle score with " .. VIC_PROG_CONFIG[vicIndex].prettyName .. " is " .. vicScore,4})
        if nextUnlockScore ~= nil then
			...
			NetEvents:SendTo('showmessage',player,{'',"You need " .. nextUnlockScore .. " more XP in " .. VIC_PROG_CONFIG[vicIndex].prettyName .. " to unlock " .. nextUnlockPrettyName,4})
        end
    ...
    elseif string.lower(message):sub(1, 6) == "!kills" then
		...
		NetEvents:SendTo('showmessage',player,{'',"You have " .. curWeapKills .. " kills with the " .. weaponProgressUnlocks[weapIndex].prettyName,4})
        if nextUnlockKills ~= nil then
           ...
           --[[ ChatManager:SendMessage(
			...
			NetEvents:SendTo('showmessage',player,{'',"You need " .. nextUnlockKills .. " more kills to unlock " .. nextUnlockPrettyName,4})
        end
...
if cPlayer['r_PlayerGuid'] == player.guid and playerIndex ~= nil then
cplayer=currentRankupPlayers[playerIndex]
pinfo={cplayer['r_PlayerLevel'],cplayer['r_AssaultLevel'],cplayer['r_EngineerLevel'],cplayer['r_SupportLevel'],cplayer['r_ReconLevel'],cplayer['r_Kills'],cplayer['r_Deaths']} -- new
NetEvents:SendTo('playerstats',player, pinfo) --new
end
end
end)
