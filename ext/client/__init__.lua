inputPress = InputDeviceKeys.IDK_Tab
active=false
Events:Subscribe('Extension:Loaded', function()
    WebUI:Init()
	WebUI:ExecuteJS(string.format('hideBox()'))
end)
Events:Subscribe('Client:UpdateInput', function(data)
	if InputManager:WentKeyDown(inputPress) and not active then
NetEvents:Send('getinfo') 
active= true
	end
	if InputManager:WentKeyUp(inputPress) then
WebUI:ExecuteJS('hideBox()')
active=false
	end
end)
...
NetEvents:Subscribe('showmessage', function(data)
 WebUI:ExecuteJS(string.format('showFadeBox('..json.encode(data)..')'))
end)
NetEvents:Subscribe('playerstats', function(data)
WebUI:ExecuteJS(string.format('playerstats(' ..json.encode(data)..')'))
WebUI:ExecuteJS(string.format('showBox()'))
end)
NetEvents:Subscribe('playsound', function(data)
WebUI:ExecuteJS(string.format('vuplayCustomSound("%s", %d, %d)', data[1], data[2], data[3]))
end)